    ...
    <div class="panel panel-default">
        <table class="table table-bordered table-hover panel-body">
            ...
            <?php foreach ($content as $idx => $row) : ?>
                <tr class="<?php echo $row->getRowStatusClass(); ?>" title="<?php echo $row->getRowTitle(); ?>"
                    row-id="<?php echo htmlspecialchars($row->getRowId()); ?>">
                    <?php if ($batch): ?>
                        <td class="batch_action rowlink-skip"><input type="checkbox" class="batch_select_row"></td>
                    <?php endif; ?>
                    <?php foreach ($columns as $column => $column_conf): ?>
                        <td class="field_<?php echo $column ?> field_class_<?php echo $column_conf['type'] ?> <?php echo @$column_conf['attributes']['class'] ?>"><?php
            ...
            <tr>
                <?php $i = 0;
                foreach ($columns as $column => $column_conf): $i++ ?>
                    <?php if ($i == 1) : ?>
...
<div class="container-fluid" id="batch-panel">
    <div class="panel panel-default">
        <div class="panel-body">
