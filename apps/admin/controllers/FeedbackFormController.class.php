...
class FeedbackFormController extends AbstractFormController{
    /** @var int ID объекта */
    private $id_object = 0;
...
//    }
    ...
    protected function processGetForm() {
        //вяжем данные
        $form = new FeedbackForm($this->context, array('id' => $this->formId));
...
//        $buttons[] = $this->getButton('save');
        ...
        if (isset($this->object['id_status']) && $this->object['id_status']!=1) {
            $buttonObj = new nomvcButtonWidget('Ответить', 'save', array('type' => 'button', 'icon' => 'ok'), array('onclick' => "TableFormActions.postForm('{$this->formId}');"));
            $buttons[] = $buttonObj->renderControl(null);
        }
...
//                 --       :id_status
//					)
//					returning id_feedback into :id_object");
//                $this->dbHelper->execute(get_class($this) . '/insert-object', $values_base, array('id_object' => &$this->id_object));
//                $values['id_dealer'] = $this->id_object;
            }
            //обновляем данные
            else {
                $id_answer = false;
                
                $this->dbHelper->addQuery(get_class($this) . '/insert-answer', "
					insert into t_feedback(
					  id_member,
					  message,
					  is_answer,
                    ...
                    returning id_feedback into :id_answer
                ");
                $values_base['id_member'] = $this->context->getUser()->getAttribute('id_member');
                $values_base['id_member_status'] = $this->context->getUser()->getAttribute('id_member');
                
                $this->dbHelper->execute(get_class($this) . '/insert-answer', $values_base, array(':id_answer' => &$id_answer));

                if ($id_answer) {
                    $values_base['id_answer'] = $id_answer;
                    
                    //send email дубль на почту, если вопрос из под дилера или АПИ
