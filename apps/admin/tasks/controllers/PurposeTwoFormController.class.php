...
class PurposeTwoFormController extends AbstractFormController{
    /** @var int ID объекта */
    private $id_object = 0;
    ...
    protected function processGetForm() {
        //вяжем данные
        $form = new PurposeTwoForm($this->context, array('id' => $this->formId));
        $form->bind($this->object);

        $formTitle = "Цель на скорость и вкус";
        ...
        if ($form->validate($request_values)) {
            ...
            //обновляем данные
                ...
                if (!$this->context->getUser()->getAttribute('id_restaurant')) {
                    ...
                    if (isset($request_values['actual_result_val']))
    ...
    private function getObject() {
        ...
        if ($this->context->getUser()->getAttribute('id_restaurant')){
            $object["actual_result_val"] = $this->getActualResult();
        }
        else{
    ...
    private function getResultsTmp(){
        $this->dbHelper->addQuery(get_class($this) . '/select-base-results-tmp', '
            ...
            round(
                ...
                /
                round(tpbrt.cnt1 / tpbrt.cnt2 * 100, 2)
                *
                100
            , 2) as result
