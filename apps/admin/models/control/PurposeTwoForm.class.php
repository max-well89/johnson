...
class PurposeTwoForm extends nomvcAbstractForm {
        ...
        */
        ...
        if ($this->isBined == true) {
            ...
            if (!$disabled && $this->getValue('id_purpose')){
                $this->addWidget(new nomvcActualResultsWidget('Фактические показатели', 'actual_results', array(
            ...
            if ($this->getValue('id_purpose')) {
                ...
                if ($disabled) {
                    if ($id_status == 3) {
                        $this->getWidget('actual_result_val')->setAttribute('disabled', true);
                    }
                }
                else{
                    if ($id_status == 3) {
//                        $this->getWidget('dt_to')->setAttribute('disabled', true);
        ...
        <script>
            function loadThresholds(){
                ...
                {'id': $('#purpose-two_id_purpose_type').val()}, function(data){
                    //var data = jQuery.parseJSON(data);
                    
                    $.each(data, function(i, item) {
                        switch (i){
                            case 0:
                                $('#purpose-two_percent_one').val(item.percent);
                                $('#purpose-two_point_one').val(item.point);
                                break;
                            case 1:
                                $('#purpose-two_percent_two').val(item.percent);
                                $('#purpose-two_point_two').val(item.point);
                                break;
                            case 2:
                                $('#purpose-two_percent_three').val(item.percent);
                                $('#purpose-two_point_three').val(item.point);
                                break;
                            default:
                                break;
                        }
            ...
            $('#purpose-two_id_purpose_type').change(function(){
                 loadThresholds();
            });
        </script>");
        }
