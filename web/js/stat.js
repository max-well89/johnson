...
var TableFormActions3 = {
    ...
    getForm: function(formClass, id, afterPostParam) {
        ...
        modal.find('.modal-footer').hide();
        modal.modal();
        $.getJSON('/backend/' + formClass + '-form/get/', { id: id }, function (data) {
            modal.find('.modal-title').html(data.title);
            modal.find('.modal-header').show();
            modal.find('.modal-body').html(data.form);
            modal.find('.modal-footer').html(data.buttons);
        ...
        if (afterPostParam) { TableFormActions3.afterPost = afterPostParam; }
        return false;
    },
    ...
    postForm: function(formClass, afterPostParam) {
        ...
        var disabled = $('form#' + formClass).find(':input:disabled').removeAttr('disabled');
        var data = $('form#' + formClass).serialize();
        disabled.attr('disabled','disabled');

        $.ajax({
            url: '/backend/' + formClass + '-form/post/',
            type: 'POST',
            data: {
                formdata: data,
                formkey: TableFormActions3.getFormKey(formClass)
            },
            success: function(answer) {
                $('#mainModal .btn').removeAttr('disabled');
                if (answer.result == 'error') {
                    ...
                    for (k in answer.fields) {
                        $('form#' + formClass + ' #form_group_' + k).addClass('has-error');
                        $('form#' + formClass + ' #form_group_' + k + ' .panel').addClass('panel-danger');
                        $('form#' + formClass + ' style.addstl').remove();
                        if ('messages' in answer && k in answer.messages) {
                            $('<style class="addstl">form#' + formClass + ' #form_group_' + k + ':before{content:"' + answer.messages[k] + '"}</style>').appendTo('form#' + formClass);
                        }
                    }
                    $('.modal.fade.in').animate({ scrollTop: $('form#bill .has-error, form#bill .panel-danger').first().offset().top - $('.modal-dialog').offset().top }, 200);
                } else {
                    if (afterPostParam) {
                        afterPostParam(formClass, answer);
                    } else {
                        TableFormActions3.closeForm(formClass, answer);
                    }
                }
            },
            dataType: 'json'
    ...
    closeForm: function(formClass) {
        if (TableFormActions3.afterPost) {
            TableFormActions3.afterPost(formClass);
        } else {
            $('#mainModal').modal('hide');
            TableFormActions3.reloadTable(formClass);
        }
        TableFormActions3.afterPost = null;
    },

    reloadTable: function(formClass) {
        $('#mainModal').modal('hide');
        $.get('/backend/' + formClass + '-table/', function (data) {
            $('.info_content').html(data);
            initChosen();
        });
    },

    deleteConfirmObject: function(formClass, afterPostParam){
        var formkey = TableFormActions3.getFormKey(formClass);
        var modal = $('#mainModal');
        modal.find('.modal-header').hide();
        modal.find('.modal-body').html('<div class="text-center"><img src="/img/loader.gif"></div>');
        modal.find('.modal-footer').hide();
        ...
        $.ajax({
            url: '/backend/' + formClass + '-object/delete-confirm/',
            type: 'POST',
    ...
    deleteObject: function(formClass) {
        $.ajax({
            url: '/backend/' + formClass + '-object/delete/',
            type: 'POST',
            data: { formkey: TableFormActions3.getFormKey(formClass) },
            success: function(answer) { TableFormActions3.reloadTable(formClass); },
            dataType: 'json'
        });
    },
    ...
    restoreObject: function(formClass) {
        $.ajax({
            url: '/backend/' + formClass + '-object/restore/',
            type: 'POST',
            data: { formkey: TableFormActions3.getFormKey(formClass) },
            success: function(answer) { TableFormActions3.reloadTable(formClass); },
            dataType: 'json'
