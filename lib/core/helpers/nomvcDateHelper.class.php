    ...
    protected function doGetDateTimeFromStr($fromFormat, $fromDate)
    {
        ...
        if (preg_match($regexpFrom, $fromDate, $matches)) {
            foreach ($symbols as $key => $sym) {
                switch ($sym) {
                    case 'd':
                        $day = intval($matches[$key + 1]);
                        break;
                    case 'm':
                        $month = intval($matches[$key + 1]);
                        break;
                    case 'Y':
                        $year = intval($matches[$key + 1]);
                        break;
                    case 'H':
                        $hour = intval($matches[$key + 1]);
                        break;
                    case 'i':
                        $minute = intval($matches[$key + 1]);
                        break;
                    case 's':
                        $second = intval($matches[$key + 1]);
                        break;
                    default:
